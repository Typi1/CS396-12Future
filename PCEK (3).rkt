...
(define unique-num 0)
(define (get-num)
  (begin (set! unique-num (+ unique-num 1))
  unique-num))

(define-metafunction PCEK
  generate-p : d -> p
  [(generate-p) ,(number->string (+ (string->number (term d)) 1))] ; also can do by counting largest p
...
(define -->PCEK
  (reduction-relation
   ...
   [--> ((let (x (cons y z)) M) E K d)
        (M (extend x (cons (lookup y E) (lookup z E)) E) K d)
        bind-cons]
   ...
   [--> (M E (activation-record_1 ((ar-t x N E_1) K_2)) d)
        (f-let (p (M E (activation-record_1 mt))) (N (extend x (ph p mt) E_1) K_2) d)
...
(define (load-PCEK p)
  (cond
    [(redex-match? PCEK M p) (term (,p () mt "0"))]
